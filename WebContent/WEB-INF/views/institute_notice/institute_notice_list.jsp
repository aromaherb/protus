...
<%
/***************************************************************************************************
 * 파일명 : institute_notice_list.html
 * 작성자 : 장재현 (jang1945@raonsolution.co.kr)
 * 작성일 : 2016-8-19
 * 내  용 : ( TB_INSTITUTE_NOTICE )의 리스트 출력/등록/상세보기/검색/페이지 이동
 ***************************************************************************************************/
%>
...
$(function() {
	...
	/**
	 * ( TB_INSTITUTE_NOTICE ) 상세보기
	 */
	$(".go_detail").click(function() {
		$("input[name=method]").val("V");
		$("input[name=notice_seq]").val($(this).attr("notice_seq"));
		$("input[name=notice_title]").val($(this).attr("notice_title"));
		$("#formList").submit();
	});
	
	/**
	 * ( TB_INSTITUTE_NOTICE ) 등록
	 */
	...
	/**
	 * ( TB_INSTITUTE_NOTICE ) formList Submit
	 */
	$("#formList").submit(function() {
		var method = $("input[name=method]").val();
		
		if ( method == "A" ) {			// 등록
			$("form[name=formList]").attr("action", "institute_notice_form.do");
		} else if ( method == "V" ) {	// 상세보기
			$("form[name=formList]").attr("action", "institute_notice_form.do");
		} else if ( method == "P" ) {	// 페이지
			$("form[name=formList]").attr("action", "institute_notice_list.do");
		} else {						// 일반 검색
			$("form[name=formList]").attr("action", "institute_notice_list.do");
					...
					<c:choose>
						<c:when test="${!empty entities}">
							<c:forEach var="entity" items="${entities}" varStatus="status">
								<tr>
									<%-- <td><a class="go_detail" href="#" notice_seq="${entity.NOTICE_SEQ}" notice_title="${entity.NOTICE_TITLE}">${entity.NOTICE_SEQ}</a></td> --%>
									<td><c:out value="${numlab}"/><c:set var="numlab" value="${numlab - 1}"/></td>
									<td><a class="go_detail" href="#" notice_seq="${entity.NOTICE_SEQ}" notice_title="${entity.NOTICE_TITLE}">${entity.NOTICE_TITLE}</td>
									<td></td>
