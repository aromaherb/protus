...
<%
/***************************************************************************************************
 * 파일명 : institute_list.html
 * 작성자 : 장재현 (jang1945@raonsolution.co.kr)
 * 작성일 : 2016-8-19
 * 내  용 : ( TB_INSTITUTE )의 리스트 출력/등록/상세보기/검색/페이지 이동
 ***************************************************************************************************/
%>
...
$(function() {
	...
	/**
	 * ( TB_INSTITUTE ) formList Submit
	 */
	$("#formList").submit(function() {
		var method = $("input[name=method]").val();
		
		if ( method == "A" ) {			// 등록
			$("form[name=formList]").attr("action", "institute_form.do");
		} else if ( method == "V" ) {	// 상세보기
			$("form[name=formList]").attr("action", "institute_form.do");
		} else if ( method == "P" ) {	// 페이지
			$("form[name=formList]").attr("action", "institute_list.do");
		} else {						// 일반 검색
			$("form[name=formList]").attr("action", "institute_list.do");
...
<div id="con_tit">
	...
	<table class="table01">
		<tr>
			...
			<th width="10%">주소</th>
			<th width="10%">주소상세</th>
			<th width="10%">사업자 등록번호</th>
			<th width="10%">연락처</th>
			<th width="10%">학원코드</th>
			<th width="10%">계좌은행</th>
			<th width="10%">예금주</th>
			<th width="10%">계좌번호</th>
			<th width="10%">대표자 이름</th>
			<th width="10%">지역코드</th>
		</tr>
	<c:choose>
	  ...
	  <c:when test="${empty entities}">
			<tr>
				<td align="center" colspan="13">검색된 자료가 없습니다.</td>
			</tr>
	  </c:when>
	</c:choose>		
