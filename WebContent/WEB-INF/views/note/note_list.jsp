 ...
 * 작성일 : 2016-8-19
 * 내  용 : ( TB_NOTE )의 리스트 출력/등록/상세보기/검색/페이지 이동
 ***************************************************************************************************/
%>
...
$(function() {
	...
	$(".go_detail").click(function() {
		...
		$("input[name=note_seq]").val($(this).attr("note_seq"));
		$("#formList").submit();
	});
	...
	/**
	 * ( TB_NOTE ) formList Submit
	 */
	$("#formList").submit(function() {
		var method = $("input[name=method]").val();
		
		if ( method == "A" ) {			// 등록
			$("form[name=formList]").attr("action", "note_form.do");
		} else if ( method == "V" ) {	// 상세보기
			$("form[name=formList]").attr("action", "note_form.do");
		} else if ( method == "P" ) {	// 페이지
			$("form[name=formList]").attr("action", "note_list.do");
		} else {						// 일반 검색
			$("form[name=formList]").attr("action", "note_list.do");
...
<div id="con_tit">
	...
	<input type="hidden" name="note_seq" value="${pageInfo.note_seq}"/>
	
	<p class="img" style="text-align:right"> 총 : <span class="point">${pageInfo.cntTotal} </span>건  |
	 현재페이지 <span class="point">${pageInfo.page} &nbsp;&nbsp; <img id="go_regist" src="<c:url value='/images/other/b-add02.gif'/>" alt="추가" /></span> </p>
	<!-- 표 내용 -->	
	<table class="table01">
		<tr>
			<th width="10%">쪽지 번호</th>
			<th width="10%">보낸이</th>
			<th width="10%">받는이</th>
			<th width="10%">읽기여부</th>
			<th width="10%">쪽지내용</th>
	...
	<c:choose>
	  <c:when test="${!empty entities}"> 
	    <c:forEach var="entity" items="${entities}" varStatus="status">
	  		<tr>
				<td><a class="go_detail" href="#" note_seq="${entity.NOTE_SEQ}" >${entity.NOTE_SEQ}</a></td>
