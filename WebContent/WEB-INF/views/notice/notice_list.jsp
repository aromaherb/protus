...
<%
/***************************************************************************************************
 * 파일명 : notice_list.html
 * 작성자 : 장재현 (jang1945@raonsolution.co.kr)
 * 작성일 : 2016-8-19
 * 내  용 : ( TB_NOTICE )의 리스트 출력/등록/상세보기/검색/페이지 이동
 ***************************************************************************************************/
%>
...
$(function() {
	...
	/**
	 * ( TB_NOTICE ) formList Submit
	 */
	$("#formList").submit(function() {
		var method = $("input[name=method]").val();
		
		if ( method == "A" ) {			// 등록
			$("form[name=formList]").attr("action", "notice_form.do");
		} else if ( method == "V" ) {	// 상세보기
			$("form[name=formList]").attr("action", "notice_form.do");
		} else if ( method == "P" ) {	// 페이지
			$("form[name=formList]").attr("action", "notice_list.do");
		} else {						// 일반 검색
			$("form[name=formList]").attr("action", "notice_list.do");
	...
	<div class="tab_con">
		<div class="board">
			...
			<form id="formList" name="formList" class="inline" method="post">
				<input type="hidden" name="page" value="${pageInfo.page}" /> <input
					type="hidden" name="method" value="" /> <input type="hidden"
					name="notice_seq" value="${pageInfo.notice_seq}" />

				<p class="img" style="text-align: right">
					총 : <span class="point">${pageInfo.cntTotal} </span>
					건 | 현재페이지 <spanz class="point">${pageInfo.page} &nbsp;&nbsp;</span>
				</p>
				<!-- 표 내용 -->
				<table class="table01">
					<tr>
						...
						<c:when test="${!empty entities}">
							<c:forEach var="entity" items="${entities}" varStatus="status">
								<tr>
									...
									<td><a class="go_detail" href="#" notice_seq="${entity.NOTICE_SEQ}">${entity.NOTICE_TITLE}</a></td>
									<td>관리자</td>
									<td></td>
