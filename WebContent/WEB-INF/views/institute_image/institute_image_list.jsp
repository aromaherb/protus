...
<%
/***************************************************************************************************
 * 파일명 : institute_image_list.html
 * 작성자 : 장재현 (jang1945@raonsolution.co.kr)
 * 작성일 : 2016-8-19
 * 내  용 : ( TB_INSTITUTE_IMAGE )의 리스트 출력/등록/상세보기/검색/페이지 이동
 ***************************************************************************************************/
%>
...
$(function() {
	...
	/**
	 * ( TB_INSTITUTE_IMAGE ) formList Submit
	 */
	$("#formList").submit(function() {
		var method = $("input[name=method]").val();
		
		if ( method == "A" ) {			// 등록
			$("form[name=formList]").attr("action", "institute_image_form.do");
		} else if ( method == "V" ) {	// 상세보기
			$("form[name=formList]").attr("action", "institute_image_form.do");
		} else if ( method == "P" ) {	// 페이지
			$("form[name=formList]").attr("action", "institute_image_list.do");
		} else {						// 일반 검색
			$("form[name=formList]").attr("action", "institute_image_list.do");
...
<div id="con_tit">
	...
	<input type="hidden" name="image_seq" value="${pageInfo.image_seq}"/>
	...
	<table class="table01">
		<tr>
			...
			<th width="10%">이미지 번호</th>
			<th width="10%">이미지 파일</th>
		</tr>
	<c:choose>
	  <c:when test="${!empty entities}"> 
	    <c:forEach var="entity" items="${entities}" varStatus="status">
	  		<tr>
				<td><a class="go_detail" href="#" institute_seq="${entity.INSTITUTE_SEQ}" image_seq="${entity.IMAGE_SEQ}" >${entity.INSTITUTE_SEQ}</a></td>
	  ...
	  <c:when test="${empty entities}">
			<tr>
				<td align="center" colspan="3">검색된 자료가 없습니다.</td>
			</tr>
	  </c:when>
	</c:choose>		
