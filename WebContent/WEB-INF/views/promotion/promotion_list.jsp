...
<%
/***************************************************************************************************
 * 파일명 : promotion_list.html
 * 작성자 : 장재현 (jang1945@raonsolution.co.kr)
 * 작성일 : 2016-8-19
 * 내  용 : ( TB_PROMOTION )의 리스트 출력/등록/상세보기/검색/페이지 이동
 ***************************************************************************************************/
%>
...
$(function() {
	...
	$(".go_detail").click(function() {
		...
		$("input[name=promotion_seq]").val($(this).attr("promotion_seq"));
		$("#formList").submit();
	});
	...
	/**
	 * ( TB_PROMOTION ) formList Submit
	 */
	$("#formList").submit(function() {
		var method = $("input[name=method]").val();
		
		if ( method == "A" ) {			// 등록
			$("form[name=formList]").attr("action", "promotion_form.do");
		} else if ( method == "V" ) {	// 상세보기
			$("form[name=formList]").attr("action", "promotion_form.do");
		} else if ( method == "P" ) {	// 페이지
			$("form[name=formList]").attr("action", "promotion_list.do");
		} else {						// 일반 검색
			$("form[name=formList]").attr("action", "promotion_list.do");
...
<div id="con_tit">
	...
	<input type="hidden" name="promotion_seq" value="${pageInfo.promotion_seq}"/>
	...
	<table class="table01">
		<tr>
			<th width="10%">프로모션 일련번호</th>
			<th width="10%">프로모션 코드</th>
			<th width="10%">시작일자</th>
			<th width="10%">종료일자</th>
			<th width="10%">사용여부</th>
			<th width="10%">사용일자</th>
	...
	<c:choose>
	  <c:when test="${!empty entities}"> 
	    <c:forEach var="entity" items="${entities}" varStatus="status">
	  		<tr>
				<td><a class="go_detail" href="#" promotion_seq="${entity.PROMOTION_SEQ}" >${entity.PROMOTION_SEQ}</a></td>
				<td>${entity.PROMOTION_CD}</td>
				<td>${entity.STATR_DATE}</td>
				<td>${entity.END_DATE}</td>
				<td>${entity.USE_YN}</td>
				<td>${entity.USE_DATE}</td>
	  ...
	  <c:when test="${empty entities}">
			<tr>
				<td align="center" colspan="7">검색된 자료가 없습니다.</td>
			</tr>
	  </c:when>
	</c:choose>		
