...
<%
/***************************************************************************************************
 * 파일명 : community_reply_list.html
 * 작성자 : 장재현 (jang1945@raonsolution.co.kr)
 * 작성일 : 2016-8-19
 * 내  용 : ( TB_COMMUNITY_REPLY )의 리스트 출력/등록/상세보기/검색/페이지 이동
 ***************************************************************************************************/
%>
...
$(function() {
	...
	/**
	 * ( TB_COMMUNITY_REPLY ) formList Submit
	 */
	$("#formList").submit(function() {
		var method = $("input[name=method]").val();
		
		if ( method == "A" ) {			// 등록
			$("form[name=formList]").attr("action", "community_reply_form.do");
		} else if ( method == "V" ) {	// 상세보기
			$("form[name=formList]").attr("action", "community_reply_form.do");
		} else if ( method == "P" ) {	// 페이지
			$("form[name=formList]").attr("action", "community_reply_list.do");
		} else {						// 일반 검색
			$("form[name=formList]").attr("action", "community_reply_list.do");
...
<div id="con_tit">
	...
	<table class="table01">
		<tr>
			<th width="10%">댓글 글쓴이</th>
			<th width="10%">글 일련번호</th>
			<th width="10%">댓글</th>
			<th width="10%">답변일자</th>
			<th width="10%">댓글 일련번호</th>
		</tr>
	<c:choose>
	  <c:when test="${!empty entities}"> 
	    <c:forEach var="entity" items="${entities}" varStatus="status">
	  		<tr>
				<td><a class="go_detail" href="#" user_seq="${entity.USER_SEQ}" community_seq="${entity.COMMUNITY_SEQ}" >${entity.USER_SEQ}</a></td>
				<td>${entity.COMMUNITY_SEQ}</td>
				<td>${entity.REPLY}</td>
				<td>${entity.REPLY_DT}</td>
				<td>${entity.REPLY_SEQ}</td>
			</tr>
	    </c:forEach>
	  </c:when>
	  <c:when test="${empty entities}">
